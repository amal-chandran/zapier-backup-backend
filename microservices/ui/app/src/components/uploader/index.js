...
class XHRUploader extends Component {
  ...
  uploadFile(file, progressCallback) {
    if (file) {
      const { xhrOptions, fieldName, method, url, onResponce } = this.props;

      const formData = new FormData();
      formData.append(this.props.fieldName, file, file.name);

      xhrOptions.headers = 'headers' in xhrOptions ? xhrOptions.headers : {};
      "Content-Type" in xhrOptions.headers ? delete xhrOptions.headers["Content-Type"] : "";
      ...
      //   if (xhrOptions.headers.hasOwnProperty(key)) {
      //     const element = xhrOptions.headers[key];
